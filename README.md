...
A full-stack Laravel platform for managing multiple communication "numbers" (channels), sending templated messages between them, and **delegating access to a number without ever sharing login credentials**.
...
**Templated messaging**
- Compose and send messages using pre-defined templates grouped by category
- A message can only be replied to using templates flagged as valid replies within the same category
- Replies are linked to the original message, forming a conversation thread
...
| Auth | `laravel/ui`, bcrypt password hashing |
...
- **Models** (`app/Models`) — `User`, `Number`, `Delegate`, `Message`, `MessageTemplate`, `MessageCategory`, `Block`. Encapsulate relationships and core business logic, e.g. `Number::isAccessibleBy()` and `Number::canReceiveFrom()`.
- **Controllers** (`app/Http/Controllers`) — handle requests and orchestrate the models. Admin-only controllers live under `Controllers/Admin` and sit behind a dedicated `admin` route group.
- **Views** (`resources/views`) — Blade templates that render the UI.
...
Request flow: `routes/web.php` → middleware (`auth`, `admin`) → controller → model/database → Blade view.
...
| `messages` | Sender number, receiver number, template used, optional parent (for threaded replies), status |

Relationships: one user → many numbers (1:N); users ↔ numbers delegation is many-to-many via `number_delegates`; a number can have many blocking rules and many messages (as sender or receiver); `messages` has a self-referencing `parent_id` for reply threads. Referential integrity is enforced with foreign keys and cascading deletes, plus unique constraints on the number value, the share token, and the owner/assistant pair.
...
- Automated unit/feature test coverage (PHPUnit scaffold is present but empty)
- Real-time message delivery (e.g. WebSockets) instead of polling
